...
class ProxSegDataset(Dataset):
    def __init__(self, data_dir, fix_orientation=False, no_obj_classes=8, train_seg_len=32,
        ...
        for seq_name in self.seq_names:
            ...
            self.all_vertices.append(self.vertices_can_dict[seq_name])
            self.all_contacts.append(self.semantics_dict[seq_name])
        
        self.all_contacts = torch.cat(self.all_contacts, dim=0) 
        self.all_vertices = torch.cat(self.all_vertices, dim=0)
    ...
    def __getitem__(self, idx):
        ret_verts_can = self.all_vertices[idx:idx+self.train_seg_len* self.jump_step: self.jump_step, : ,: ]
        contacts_s = self.all_contacts[idx:idx+self.train_seg_len* self.jump_step, : ]
        ...
        # print(verts_can.shape, contacts_s.shape)
