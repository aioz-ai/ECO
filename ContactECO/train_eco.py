...
import numpy as np
from ContactECO.eco import ECO, convert_me_to_torch
...
def train():
    ...
    if use_dataset == "default":
        for verts_can, contacts_s in tqdm(train_data_loader):
            ...
            # c =  verts_can.shape[0]
            # if c < batch_size: 
            #   verts_can = F.pad(input=verts_can, pad=(0, 0, 0, 0, 0, batch_size - c), mode='constant', value=0)
            #   gt_cf = F.pad(input=gt_cf, pad=(0, 0, 0, 0, 0, batch_size - c), mode='constant', value=0)
            optimizer.zero_grad()
            # pr_cf: (bs, 655, 8), mu: (bs, 256), logvar: (bs, 256)
            gt_cf1 = torch.clone(gt_cf)

            #forward
            pr_cf, mu, logvar = model(gt_cf, verts_can)
            recon_loss_semantics, semantics_recon_acc = compute_recon_loss_posa(gt_cf1, pr_cf, **args_dict)
    ...
    with torch.no_grad():
        ...
        if use_dataset == "default":
            for verts_can, contacts_s in tqdm(valid_data_loader):
                ...
                # convert z and vertices to ME tensor
                z = z.unsqueeze(-1)
                z = ME.to_sparse_all(z)
                pr_cf_list = []
                # scale the weight contr to (0,1)
                w_ver =  F.softmax(model.vertices_contr.weight.data, dim = 0)
                w_ver = torch.clone(model.vertices_contr.weight.data)
                
                for i in range(model.num_mask): 
                  sparse_ver_i = torch.clone(verts_can) 
                  sparse_ver_i[:,model.vertices_mask[i],:] = 0
                  ver_i = w_ver[i][0]*(sparse_ver_i)
                ...
                pr_cf = torch.stack(pr_cf_list, dim=0).sum(dim=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--out_dir", type=str, default="../3 masks/")
    parser.add_argument("--experiment", type=str, default="custom_posa09")
    ...
    dtype = torch.float32
    kl_w = 0.05
    model = ECO(num_mask = num_mask, sparsity_ratio = sparsity, ds_us_dir=mesh_ds_dir, use_semantics=True, channels=f_vert).to(device)
    # these layers are used to multiply the two SparseTensor, so no need to backward
    model.encoder.en_spiral[2].layer.requires_grad_(False) 
    model.encoder.en_spiral[4].layer.requires_grad_(False)


    # checkpoint = torch.load("OUTPUT_DIR_3_09-bs_4/EXPERIMENT/model_ckpt/best_model_recon_acc.pt")
    ...
    valid_data_loader = DataLoader(valid_dataset, batch_size=1, shuffle=False, num_workers=num_workers)
    ...
    for e in range(starting_epoch, epochs):
        ...
        if save_ckpt and e % save_interval == save_interval - 1:
            data = {
                ...
                'weight_contr': model.vertices_contr.weight.data 
            }
